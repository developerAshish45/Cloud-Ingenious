...
export default class ReachInCases extends LightningElement {
    ...
    generateSections() {
        this.sections = [];
        let section = { key: 1, label: 'Case In Reach '+this.count, rows: [] };
        for (let j = 1; j <= 3; j++) {
            let row = { key: j, columns: [] };
            for (let k = 1; k <= 3; k++) {
                if (k === 1) {
                    row.columns.push({ key: k, label: this.column1Labels[j - 1], isInput: true, sufix: this.type + this.sufixName[j - 1] + ':' + 'RC-' +this.count});
                } else if (k === 2) {
                    row.columns.push({ key: k, label: 'File Upload ' + k, isFileUpload: true, sufix: this.type + this.sufixName[j - 1] + ':' + 'RC-' +this.count + 'RecordId:' + this.surveyRecord });
                } else {
