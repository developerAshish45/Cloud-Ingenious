...
export default class LowTempEvaporators extends LightningElement {
    ...
    generateSections() {
        ...
        for (let i = 1; i <= parseInt(this.selectedSectionCount); i++) {
            ...
            for (let j = 1; j <= 11; j++) {
                ...
                for (let k = 1; k <= 3; k++) {
                    if (k === 1) {
                        row.columns.push({ key: k, label: this.column1Labels[j - 1] , isInput: true , sufix : 'LowTemp'+ this.sufixName[j - 1] + ':' + 'EV-' + i});
                    } else if (k === 2) {
                        row.columns.push({ key: k, label: 'File Upload ' + k, isFileUpload: true , sufix : 'LowTemp'+ this.sufixName[j - 1] + ':' + 'EV-' + i +'RecordId:' + this.surveyRecord});
                    } else {
