...
import getSurveyRecordApex from '@salesforce/apex/SurveyorPortalController.getSurveyRecordGen';
...
export default class RsgSurveyDataMT extends LightningElement {
   @api surveyRec ;
    surveyRecord = [];
    isShelves = false;
    isNoteDataLoading = false;
    get options() {
        return [
            { label: 'Yes', value: 'Yes' },
            { label: 'No', value: 'No' },
   ...
   @track noteBody ;
    ...
    handleAddNoteClick(event) {
        ...
        const inputElement = button.closest('.slds-form-element').querySelector('lightning-input');
        if (inputElement) {
            const fieldValue = inputElement.dataset.field;
    ...
    handleGetNote(){
        console.log('LIne 152 ', this.surveyRec);
