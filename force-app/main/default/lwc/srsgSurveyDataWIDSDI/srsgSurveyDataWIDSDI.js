...
import RSGCoolerShelves from '@salesforce/resourceUrl/RSGDoorShelves';
import RSGCoolerShelves2 from '@salesforce/resourceUrl/RSGDoorShelves2';
export default class SrsgSurveyDataWIDSDI extends LightningElement {
    @api surveyRec ;
    surveyRecord = [];
    isShelves = false;
    isShelvesLowTemp = false;
    isShelvesBeerCooler = false;
    isPopupVisible = false;
    fieldName = '';
    imgDoorShelves = RSGCoolerShelves;
    imgDoorShelves2 = RSGCoolerShelves2;
   @track noteBody ;
    noteTitle ;
    get options() {
        return [
        ...
        .then(result => {
            if(result != null){
                ...
                if(this.surveyRecord.Brkn_Or_Missing_Cooler_Or_FS_MediumTemp__c == 'Yes'){
                 ...
                 if(this.surveyRecord.Brkn_Or_Missing_Cooler_Or_FS_LowTemp__c == 'Yes'){
                    this.isShelvesLowTemp = true;
                 }else{
                    this.isShelvesLowTemp = false;
                 }
                 if(this.surveyRecord.Brkn_Or_Missing_Cooler_Or_FS_BeerCooler__c == 'Yes'){
                    this.isShelvesBeerCooler = true;
                 }else{
                    this.isShelvesBeerCooler = false;
                 }
