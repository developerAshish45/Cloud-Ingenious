...
import getGenRecords from '@salesforce/apex/InstallerDetailFormController.getGenRecords';
import updateGenData from '@salesforce/apex/InstallerDetailFormController.updateGenData';
...
export default class InstallerGenChecklist extends NavigationMixin (LightningElement) {
    @api recordId;
    @api accountId;
    hasChecked = false;
    alert;
    ...
    handleSubmit() {
        ...
        try {
            ...
            if (!this.hasChecked) {
                ...
                updateGenData({recId: this.recordId, checkboxSelectList: this.selectedCons, status: 'In Progress'})
                .then(result => {
                    this.isShowModal = true;
                    this.alert = 'Alert';
                ...
                updateGenData({recId: this.recordId, checkboxSelectList: this.selectedCons, status: 'Submitted'})
                .then(result => {
    ...
    handleDisplayChecklist(checklist) {
        getGenRecords({checklistSection: checklist})
        .then(result => {
...
//Modal JS
    handleAddNoteClick(event) {
        ...
        if (this.isPopupVisible == true) {
            ...
            this.checkboxCounter = this.checkboxCounter + parseInt(1);
