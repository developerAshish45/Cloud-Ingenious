...
export default class BeerTempReachCase extends LightningElement {
    ...
    generateSections() {
        ...
        for (let i = 1; i <= parseInt(this.selectedSectionCount); i++) {
            ...
            for (let j = 1; j <= 3; j++) {
                ...
                for (let k = 1; k <= 3; k++) {
                    if (k === 1) {
                        row.columns.push({ key: k, label: this.column1Labels[j - 1] , isInput: true , sufix : 'BeerTemp'+ this.sufixName[j - 1] + ':' + 'RC-' + i});
                    } else if (k === 2) {
                        row.columns.push({ key: k, label: 'File Upload ' + k, isFileUpload: true , sufix : 'BeerTemp'+ this.sufixName[j - 1] + ':' + 'RC-' + i +'RecordId:' + this.surveyRecord});
                    } else {
